...
def run_dmr_model(gridL, lineL, status_placeholder, num_topics, seed, iterations, alpha, eta):
    ...
    st.write(f"Training DMR Model with {num_topics} topics, seed={seed}, alpha={alpha}, eta={eta}...")
    progress_bar = st.progress(0)
    log_area = st.empty()
    log_text = ""
    for i in range(0, iterations, 20):
        model.train(20, workers=1)
        log_text += f"Iteration: {i + 20}/{iterations}\tLog-likelihood: {model.ll_per_word:.4f}\n"
        log_area.code(log_text)
        progress_bar.progress((i + 20) / iterations)
    topics = [model.get_topic_words(i, top_n=20) for i in range(model.k)]
    doc_dists = [doc.get_topic_dist() for doc in model.docs]
    dt_df = pd.DataFrame(doc_dists, columns=[f'T{i}' for i in range(num_topics)], index=range(len(doc_dists)))
    return model, topics, dt_df

# --- Streamlit UI를 표시하는 메인 함수 ---
def show():
    # --- ✨ 복원된 파트 1: 일반 텍스트 유사도 분석 ---
    with st.expander("파트 1: 일반 텍스트 벡터화 및 유사도 분석", expanded=True):
        st.subheader("1. 분석할 문서 입력")
        default_texts = "저는 사과 좋아요\n저는 바나나 좋아요\n저는 바나나 좋아요 저는 바나나 좋아요"
        text_input = st.text_area("분석할 문장들을 한 줄에 하나씩 입력하세요.", value=default_texts, height=100, key="sim_text_input")
        ...
        if texts:
            ...
            if st.session_state.get('count_dtm_df') is not None:
                st.write("#### 문서-단어 행렬 (DTM)"); st.dataframe(st.session_state.count_dtm_df, use_container_width=True)
                st.write("#### 각 문서의 특성 벡터"); 
                for i, row in st.session_state.count_dtm_df.iterrows(): st.text(f"{i}: {list(row)}")
                st.write("#### 문서 간 코사인 유사도"); st.dataframe(st.session_state.cos_sim_df, use_container_width=True)
            ...
            if st.session_state.get('tfidf_dtm_df') is not None:
                st.write("#### 문서-단어 행렬 (DTM) - TF-IDF"); st.dataframe(st.session_state.tfidf_dtm_df, use_container_width=True)
                st.write("#### 문서 간 코사인 유사도 (TF-IDF 기반)"); st.dataframe(st.session_state.tfidf_cos_sim_df, use_container_width=True)

    st.divider()

    # --- 파트 2: '개벽' 특성 벡터 추출 (변경 없음) ---
    st.header("4. 개벽의 특성 벡터 추출")
    st.info("1, 2단계에서 생성/전처리한 데이터 또는 유사한 구조의 파일을 업로드하여 텍스트의 특성을 추출합니다.")
    
    uploaded_file_tab4 = st.file_uploader("특성 추출에 사용할 엑셀 파일을 업로드하세요 (doc_split_12gram, grid_1 열 필요).", type=['xlsx', 'xls'], key="tab4_file_uploader")

    if uploaded_file_tab4:
        df_for_feature = pd.read_excel(uploaded_file_tab4)
        
        with st.expander("4.1. '고빈도 단어'를 사용하는 경우", expanded=True):
            stopw_input = st.text_input("제외할 단어(Stopwords)를 쉼표(,)로 구분하여 입력하세요.", value="문제,금일,관계")
            rank_n_input = st.number_input("추출할 고빈도 단어 개수를 입력하세요.", min_value=10, max_value=200, value=50, step=10)
            if st.button("고빈도 단어 특성 추출 실행"):
                stopw = [word.strip() for word in stopw_input.split(',')]
                with st.spinner("DTM 생성 및 고빈도 단어 추출 중..."):
                    st.session_state.highword_df = get_dtm_gb(df_for_feature, 'doc_split_12gram', stopw, rank_n_input)
            if st.session_state.get('highword_df') is not None:
                st.success(f"상위 {rank_n_input}개 고빈도 단어 DTM이 생성되었습니다."); st.dataframe(st.session_state.highword_df, use_container_width=True)
        ...
        with st.expander("4.2. '토픽'을 사용하는 경우", expanded=True):
            ...
            with p_col1:
                ...
                alpha_input = st.number_input("알파 (alpha)", min_value=0.01, max_value=1.0, value=0.05, step=0.01, format="%.2f")
            with p_col2:
                iterations_input = st.number_input("반복 횟수 (iterations)", min_value=100, max_value=5000, value=1000, step=100)
                eta_input = st.number_input("에타 (eta)", min_value=0.01, max_value=1.0, value=0.1, step=0.01, format="%.2f")
            with p_col3:
            ...
            if st.button("토픽 모델링 특성 추출 실행"):
                ...
                model, topics, dt_df = run_dmr_model(gridL, lineL, status_placeholder,
            ...
            if st.session_state.get('topic_df') is not None:
                ...
                topic_word_df = pd.DataFrame([[word[0] for word in topic] for topic in st.session_state.topics_words], index=[f"Topic {i}" for i in range(len(st.session_state.topics_words))]).transpose()
                st.dataframe(topic_word_df, use_container_width=True)
                st.write("**문서별 토픽 분포 (특성 벡터):**"); st.dataframe(st.session_state.topic_df, use_container_width=True)